...
- `datetime`: Date and time when the email was sent, as an ISO 8601 timestamp
  carrying the Urbana-Champaign offset, e.g. `2026-09-09T14:31:00-0500`
  (`-0600` outside daylight saving). Read it with `tz = "America/Chicago"` to
  get the local wall clock back; a bare `read_csv()` will show it in UTC.
- `date`: Date when the email was sent in the form of `YYYY-MM-DD`, local to
  Urbana-Champaign.
- `time`: Time when the email was sent in the form of a 24 hour clock with
  `HH:MM`, zero padded, local to Urbana-Champaign.
