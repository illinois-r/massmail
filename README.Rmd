...
- `datetime`: Date and time when the e-mail was sent in the form of `YYYY-MM-DD HMS` 
  with either CDT or CST.
- `date`: Date when the e-mail was sent in the form of `YYYY-MM-DD`.
- `time`: Time when the e-mail was sent in the form of a 24 hour clock with `HH:MM`.
...
```{r show-preview, include = FALSE}
library("tidyverse")
id = massmail_data %>%
  dplyr::filter(url == "https://massmail.illinois.edu/massmail/1930327.html")
...
```{r pkg-dependencies, message = FALSE, warning = FALSE} 
# Script dependencies ----
